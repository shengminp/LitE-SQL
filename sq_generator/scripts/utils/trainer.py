...
from .data import load_and_prepare_revise_data
from .metric import (
    post_process,
    post_process_spider,
...
class T2STrainer(Trainer):
    ...
    def get_eval_dataloader(self, eval_dataset: Optional[Union[str, Dataset]] = None) -> DataLoader:
        ...
        data_collator = DataCollatorForSeq2Seq(self.processing_class, padding=True)
    ...
    def evaluation_loop(
        ...
        def _run_dataloader(
            ...
            return all_losses, all_preds, all_labels, all_inputs
        ...
        all_losses, all_preds, all_labels, all_inputs = _run_dataloader(
            batch_size, 
            dataloader,
            all_losses,
            all_preds,
            all_labels,
            all_inputs,
            args.sql_generation_config)
        
        if args.eval_mode == "revise":
            logger.info(f"** Revising SQL **")
            from torch.distributed import is_initialized
            if is_initialized():
                self.accelerator.wait_for_everyone()
            if self.is_world_process_zero():
                total_results = post_process('sft', all_preds, all_labels, self.processing_class)
                ...
                sql_df = collect_result_parallel(args, 'test', sql_df)
            else:
                sql_df = None
                
            sql_df_list = [sql_df] if self.is_world_process_zero() else [None]
            from accelerate.utils import broadcast_object_list
            sql_df_list = broadcast_object_list(sql_df_list)
            sql_df = sql_df_list[0]

            correct_df = sql_df[sql_df['res'] == 1]
            incorrect_df = sql_df[sql_df['res'] == 0]
            if len(incorrect_df) != 0:
                revise_dataset = load_and_prepare_revise_data(incorrect_df, self.processing_class, args)
                revise_dataloader = self.get_eval_dataloader(revise_dataset)
                ...
                print("Nothing need to be revised")
                revise_dataset = self.eval_dataset
...
class SDPOTrainer(T2STrainer):
    ...
    def concatenated_inputs(
        batch: dict[str, Union[list, torch.LongTensor]], padding_value: int, processing_class
    ...
    def flush_right(
        ...
        for i in range(mask.size(0)):
            last_one_idx = torch.nonzero(mask[i])[-1].item()                                
            mask[i] = torch.roll(mask[i], shifts=(mask.size(1) - 1) - last_one_idx)
            for tensor in tensors:
                tensor[i] = torch.roll(tensor[i], shifts=(mask.size(1) - 1) - last_one_idx)

        empty_cols = torch.sum(mask, dim=0) == 0
        first_non_empty_from_right = torch.nonzero(~empty_cols)
        first_empty_col = 0
        if first_non_empty_from_right.numel():
            first_empty_col = first_non_empty_from_right[0].item()
    ...
    def concatenated_forward(self, model: nn.Module, batch: dict[str, Union[list, torch.LongTensor]]):
        ...
        concatenated_batch = self.concatenated_inputs(batch, padding_value=self.padding_value, processing_class=self.processing_class)
        ...
        if self.is_encoder_decoder:
            ...
            # [[0, 0, x, x, x, x],  ->  [[x, x, x, x],
            #  [0, x, x, x, 0, 0]]       [0, x, x, x]]
            attention_mask, input_ids, loss_mask = self.flush_right(attention_mask, input_ids, loss_mask)
